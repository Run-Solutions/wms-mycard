...
const CorteComponent: React.FC<Props> = ({ workOrder, currentFlow }) => {
  const navigation =
  ...
  useEffect(() => {
    ...
    if (corte && partials.length === 0) {
      ...
      setInconformityValues({
        ...
        inconformity: corte.formAuditory?.inconformities.at(-1)?.comments || '',
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      await acceptCorteInconformityAuditory(areaResponseFlowId);
      setShowModal(false);
  ...
  console.log(sumaBadQuantity, 'sumabad')
