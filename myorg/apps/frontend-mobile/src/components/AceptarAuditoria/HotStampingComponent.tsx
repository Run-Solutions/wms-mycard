...
const HotStampingComponentAcceptAuditory: React.FC<{ workOrder: any }> = ({ workOrder }) => {
  ...
  useEffect(() => {
    if (workOrder?.areaResponse?.hotStamping && workOrder?.partialReleases.length === 0) {
      const vals: HotStampingData = {
        good_quantity: String(workOrder.areaResponse.hotStamping.good_quantity ?? "0"),
        bad_quantity: String(workOrder.areaResponse.hotStamping.bad_quantity ?? "0"),
        excess_quantity: String(workOrder.areaResponse.hotStamping.excess_quantity ?? "0"),
