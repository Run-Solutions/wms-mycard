...
const MillingChipComponentAcceptAuditory: React.FC<{ workOrder: any }> = ({ workOrder }) => {
  ...
  useEffect(() => {
    if (workOrder?.areaResponse?.millingChip && workOrder?.partialReleases.length === 0) {
      const vals: MillingChipData = {
        good_quantity: String(workOrder.areaResponse.millingChip.good_quantity ?? "0"),
        bad_quantity: String(workOrder.areaResponse.millingChip.bad_quantity ?? "0"),
        excess_quantity: String(workOrder.areaResponse.millingChip.excess_quantity ?? "0"),
...
export default MillingChipComponentAcceptAuditory;
