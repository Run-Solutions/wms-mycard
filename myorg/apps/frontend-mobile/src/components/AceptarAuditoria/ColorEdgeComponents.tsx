...
const ColorEdgeComponentAcceptAuditory: React.FC<{ workOrder: any }> = ({ workOrder }) => {
  ...
  useEffect(() => {
    if (workOrder?.areaResponse?.colorEdge && workOrder?.partialReleases.length === 0) {
      const vals: ColorEdgeData = {
        good_quantity: String(workOrder.areaResponse.colorEdge.good_quantity ?? "0"),
        bad_quantity: String(workOrder.areaResponse.colorEdge.bad_quantity ?? "0"),
        excess_quantity: String(workOrder.areaResponse.colorEdge.excess_quantity ?? "0"),
