...
const CorteComponentAcceptAuditory: React.FC<{ workOrder: any }> = ({ workOrder }) => {
  ...
  useEffect(() => {
    if (workOrder?.areaResponse?.corte && workOrder?.partialReleases.length === 0) {
      const vals: CorteData = {
        good_quantity: String(workOrder.areaResponse.corte.good_quantity ?? "0"),
        bad_quantity: String(workOrder.areaResponse.corte.bad_quantity ?? "0"),
        excess_quantity: String(workOrder.areaResponse.corte.excess_quantity ?? "0"),
