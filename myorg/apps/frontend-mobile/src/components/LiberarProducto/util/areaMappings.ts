...
// myorg/apps/frontend-mobile/src/components/LiberarProducto/util/areaMappings.ts
...
export async function loadBadQuantitySummaryAsync(otId: string, flowId: number) {
  try {
    if (typeof AsyncStorage?.getItem !== 'function') return [];
    const key = `bqs:${otId}:${flowId}`;
    const raw = await AsyncStorage.getItem(key);
    return raw ? JSON.parse(raw) : [];
  } catch (e) {
    console.error('Error reading bad quantity summary', e);
    return [];
  }
}

export async function saveBadQuantitySummaryAsync(
  otId: string,
  flowId: number,
  data: any[]
) {
  try {
    ...
    console.error('Error writing bad quantity summary', e);
  }
}
