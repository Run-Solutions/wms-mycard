...
const CardPreview: React.FC<{
  ...
  return (
    ...
    >
      ...
      <Overlay className="overlay">{isImage ? 'Ver imagen' : 'Ver PDF'}</Overlay>
    </PreviewBox>
  );
};
...
const AcceptProductPage: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'priority' | 'createdAt' | 'quantity' | 'ot' | 'mycard'>('priority');
      ...
      {isModalOpen && selectedOrder && (
        <ModalOverlay onClick={closeModal}>
          <ModalContent onClick={(e) => e.stopPropagation()}>
                  ...
                  {selectedOrder.workOrder.files.map((file) => {
                    ...
                    return (
                      <button
                        ...
                        className="flex items-center justify-center w-full px-3 py-2 rounded-lg shadow-sm bg-white hover:bg-gray-100 border text-sm font-medium transition-all duration-200"
                      >
                        <span>{label}</span>
                      </button>
                    );
            ...
            <button style={{ marginTop: '20px', backgroundColor: '#bbbbbb' }} onClick={closeModal}>Cerrar</button>
            <button
              style={{ marginTop: '20px', backgroundColor: '#0038A8', color: 'white' }}
              onClick={aceptarOT}
            >
              Aceptar OT
            </button>
            ...
            filteredSorted.map((order) => {
              ...
              return (
                ...
                >
                  ...
                  <Info>
                    Fecha de creación: {new Date(workOrder.createdAt).toLocaleDateString()}
                  </Info>
                </WorkOrderCard>
              );
            })
