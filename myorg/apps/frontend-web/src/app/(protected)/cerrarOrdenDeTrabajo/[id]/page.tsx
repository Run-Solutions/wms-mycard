...
export default function CloseWorkOrderAuxPage({ params }: Props) {
  ...
  const renderCell = (area: AreaData, field: NumericField) => {
    ...
    if (!canOpenInStatus(area.status))
      return <span>{Number(area[field] ?? 0)}</span>;
    ...
    if (field === 'malas' && canOpenInStatus(area.status) && area.id >= 6) {
      if (area.id >= 6) {
        ...
        const selfMat = toNum(
          area?.response?.[blockKey]?.material_quantity ?? 0
        );
        console.log(area);
        console.log(selfBad, 'selfBad');
                            ...
                            if (rem > 0) {
                              ...
                              switch (field) {
                                ...
                                case 'noprocess':
                                  ...
                                  break;
                                case 'malas': {
                                  const { remBad, sumBad } = getRemTotal(
                                    area.id
                                  );
                              ...
                              field === 'malas' &&
                              area.id >= 6 &&
                              (area.status === 'Completado' || area.status.includes('auditoria'));
