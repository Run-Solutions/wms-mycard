...
export default function HotStampingComponent({ workOrder }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/inconformities/${areaResponse}/hot-stamping`, {
        method: 'PATCH',
        headers: {
    ...
    <Container>
      <NewData>
        ...
        <NewDataWrapper>
          <InputGroup>
          <Label>Buenas:</Label>
              <Input type="number" name="good_quantity" value={workOrder.areaResponse.hotStamping.good_quantity} disabled/>
              <Label>Malas:</Label>
              <Input type="number" name="bad_quantity" value={workOrder.areaResponse.hotStamping.bad_quantity} disabled/>
              <Label>Exceso:</Label>
              <Input type="number" name="excess_quantity" value={workOrder.areaResponse.hotStamping.excess_quantity} disabled/>
          </InputGroup>
        </NewDataWrapper>
        <InputGroup>
