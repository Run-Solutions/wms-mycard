...
export default function LaminacionComponent({ workOrder }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/inconformities/${areaResponse}/laminacion`, {
        method: 'PATCH',
        headers: {
      ...
      <NewData>
        ...
        <NewDataWrapper>
          <InputGroup>
            ...
            <Input type="number" value={workOrder.areaResponse.laminacion.release_quantity} disabled/>
          </InputGroup>
        </NewDataWrapper>
        <InputGroup>
          <Label>Comentarios</Label>
          <Textarea value={workOrder.areaResponse.laminacion.comments} disabled/>
        </InputGroup>
      </NewData>
    </Container>
    <Container>
