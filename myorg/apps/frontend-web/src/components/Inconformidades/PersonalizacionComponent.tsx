...
export default function PersonalizacionComponent({ workOrder }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/inconformities/${areaResponse}/personalizacion`, {
        method: 'PATCH',
        headers: {
    ...
    <Container>
      <NewData>
        ...
        <NewDataWrapper>
          <InputGroup>
          <Label>Buenas:</Label>
              <Input type="number" name="good_quantity" value={workOrder.areaResponse.personalizacion.good_quantity} disabled/>
              <Label>Malas:</Label>
              <Input type="number" name="bad_quantity" value={workOrder.areaResponse.personalizacion.bad_quantity} disabled/>
              <Label>Exceso:</Label>
              <Input type="number" name="excess_quantity" value={workOrder.areaResponse.personalizacion.excess_quantity} disabled/>
          </InputGroup>
        </NewDataWrapper>
        <InputGroup>
          <Label>Comentarios</Label>
          <Textarea value={workOrder.areaResponse.personalizacion.comments} disabled/>
        </InputGroup>
      </NewData>
    </Container>
    <Container>
