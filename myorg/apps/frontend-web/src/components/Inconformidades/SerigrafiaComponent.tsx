...
export default function SerigrafiaComponent({ workOrder }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/inconformities/${areaResponse}/serigrafia`, {
        method: 'PATCH',
        headers: {
      ...
      <NewData>
        ...
        <NewDataWrapper>
          <InputGroup>
            ...
            <Input type="number" value={workOrder.areaResponse.serigrafia.release_quantity} disabled/>
          </InputGroup>
        </NewDataWrapper>
        <InputGroup>
          <Label>Comentarios</Label>
          <Textarea value={workOrder.areaResponse.serigrafia.comments} disabled/>
        </InputGroup>
      </NewData>
    </Container>
    <Container>
