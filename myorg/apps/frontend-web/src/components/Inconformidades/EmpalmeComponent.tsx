...
export default function EmpalmeComponent({ workOrder }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/inconformities/${areaResponse}/empalme`, {
        method: 'PATCH',
        headers: {
      ...
      <NewData>
        ...
        <NewDataWrapper>
          <InputGroup>
            ...
            <Input type="number" value={workOrder.areaResponse.empalme.release_quantity} disabled/>
          </InputGroup>
        </NewDataWrapper>
        <InputGroup>
          <Label>Comentarios</Label>
          <Textarea value={workOrder.areaResponse.empalme.comments} disabled/>
        </InputGroup>
      </NewData>
    </Container>
    <Container>
