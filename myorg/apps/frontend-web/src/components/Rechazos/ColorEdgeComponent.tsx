...
export default function ColorEdgeComponent({ workOrder, currentFlow }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await acceptColorEdgeInconformityAuditory(areaResponseFlowId);
  ...
  useEffect(() => {
    ...
    console.log('ColorEdge:', currentFlow);
    ...
    if (colorEdge && partials.length === 0) {
      ...
      setInconformityValues({
        ...
        user:
          colorEdge.formAuditory?.inconformities.at(-1)?.user.username || '',
        inconformity:
          colorEdge.formAuditory?.inconformities.at(-1)?.comments || '',
      });
