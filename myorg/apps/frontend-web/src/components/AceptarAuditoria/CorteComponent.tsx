...
export default function CorteComponentAcceptAuditory({ workOrder }: Props) {
  ...
  if (workOrder.area_id >= 2) {
    ...
    const [currentValues, setCurrentValues] = useState<CorteData>({
      good_quantity: "",
      bad_quantity: "",
      excess_quantity: "",
      comments: "",
    ...
    useEffect(() => {
      if (workOrder?.areaResponse?.corte) {
    ...
    const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const key = name as keyof CorteData;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      const CorteId = workOrder.areaResponse.corte.id;
      console.log(CorteId);
      try {
