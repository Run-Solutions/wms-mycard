...
export default function PersonalizacionComponentAcceptAuditory({ workOrder }: Props) {
  ...
  if (workOrder.area_id >= 2) {
    ...
    const [currentValues, setCurrentValues] = useState<PersonalizacionData>({
      good_quantity: "",
      bad_quantity: "",
      excess_quantity: "",
      comments: "",
    ...
    const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const key = name as keyof PersonalizacionData;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      console.log(personalizacionId);
      try {
        const res = await fetch(`http://localhost:3000/work-order-flow-auditory/personalizacion/${personalizacionId}`, {
          method: 'POST',
          headers: {
