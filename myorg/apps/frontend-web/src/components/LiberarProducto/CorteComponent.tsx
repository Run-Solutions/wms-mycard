    ...
    <Container>
      ...
      <NewData>
        ...
        <NewDataWrapper>
          ...
          <CqmButton status={workOrder.status} onClick={openModal} disabled={['Enviado a CQM', 'En Calidad', 'Listo', "Enviado a auditoria parcial"].includes(workOrder.status)}>Enviar a CQM</CqmButton>
        </NewDataWrapper>
        <InputGroup>
      ...
      <LiberarButton disabled={isDisabled || ['Enviado a CQM', 'En Calidad', 'Enviado a auditoria parcial', 'Parcial'].includes(workOrder.status) || ['En calidad', 'Parcial', 'Pendiente parcial', 'En auditoria', 'Enviado a auditoria'].includes(nextFlow?.status) && !allParcialsValidated} onClick={handleLiberarClick}>Liberar Producto</LiberarButton>
    </Container>
...
const CqmButton = styled.button<CqmButtonProps>`
  ...
  background-color: ${({ status }) => {
    ...
    if (['Enviado a CQM', 'En Calidad', 'Enviado a auditoria parcial'].includes(status)) return '#9ca3af'; // gris
    return '#2563eb'; // azul
  }};
  color: white;
  padding: 0.75rem 2rem;
  ...
  transition: background 0.3s;
  cursor: ${({ status }) => {
    if (['Enviado a CQM', 'En Calidad', 'Listo', 'Enviado a auditoria parcial'].includes(status))
      return 'not-allowed';
    return 'pointer';
  }};
