...
export default function MillingChipComponent({ workOrder }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:3000/free-order-flow/cqm-milling-chip', {
        method: 'POST',
        headers: {
  ...
  const handleImpressSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:3000/free-order-flow/milling-chip', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
      ...
      <ModalOverlay>
        <ModalContent>
          ...
          <InputGroup style={{ paddingTop: '30px', width: '70%'}}>
            ...
            <Input type="text" placeholder="Ej: " value={revisarTecnologia} onChange={(e) => setRevisarTecnologia(e.target.value)}/>
          </InputGroup>
