...
export class CloseAuditoryWorkOrderController {
  constructor(
    private readonly CloseAuditoryWorkOrderService: CloseAuditoryWorkOrderService,
  ) {}

  // Para obtener los WorkOrderFlowEnAuditoria
  @Get('in-auditory')
  async getInAuditoryWorkOrders(
    ...
    return await this.CloseAuditoryWorkOrderService.getInAuditoryWorkOrders(
      user.id,
      statuses,
    );
  }
  ...
  @Get(':id')
  async getInAuditoryWorkOrderById(
    @Param('id') id: string,
    @Req() req: AuthenticatedRequest,
  ) {
    const user = req.user;
    ...
    return await this.CloseAuditoryWorkOrderService.getInAuditoryWorkOrderById(
      id,
    );
  }
