...
export class AcceptWorkOrderController {
  constructor(
    private readonly AcceptWorkOrderService: AcceptWorkOrderService,
  ) {}
  ...
  async getPendingWorkOrders(@Req() req: AuthenticatedRequest) {
    ...
    return await this.AcceptWorkOrderService.getPendingWorkOrders(
      user.areas_operator_id,
    );
  }
  ...
  async getInconformidadWorkOrders(
    ...
    const statuses = statusesRaw
      ...
      : ['En inconformidad'];
    return await this.AcceptWorkOrderService.getInconformidadWorkOrders(
      user.role_id,
      user.areas_operator_id,
      statuses,
      user.id,
  ...
  async inconformidadWorkOrderFlow(
    @Param('id') id: number,
    @Req() req: AuthenticatedRequest,
    @Body() body: { inconformidad: string },
  ) {
    const userId = req.user?.id;
    if (!userId) throw new UnauthorizedException();

    const updatedFlow =
      await this.AcceptWorkOrderService.inconformidadWorkOrderFlow(
        +id,
        userId,
        body.inconformidad,
      );
    return {
      message: 'Orden con inconformidad registrada correctamente',
      flow: updatedFlow,
    };
  }

  // Para que el calidad marque la respuesta entregada con inconformidad
  ...
  async inconformidadCQMWorkOrderFlow(
    ...
    const updatedFlow =
      await this.AcceptWorkOrderService.inconformidadCQMWorkOrderFlow(
        +id,
        userId,
        body.inconformidad,
      );
  ...
  // Para que el operador marque la entrega de la orden de auditoria con inconformidad
  @Patch(':id/inconformidad-auditoria')
  async inconformidadAuditoryWorkOrderFlow(
    ...
    const updatedFlow =
      await this.AcceptWorkOrderService.inconformidadAuditoryWorkOrderFlow(
        +id,
        userId,
        body.inconformidad,
      );
  ...
  async getWorkOrderFlowById(
    ...
    return await this.AcceptWorkOrderService.getWorkOrderFlowById(
      id,
      user.areas_operator_id,
    );
  }
